...
subs_sim <- function(params) {
  ...
  rest <- sim(base_soln, Tmax = params$Tmax - params$Tsubs, x0 = xi) %>% 
    filter(time!=0) %>% mutate(time = time + params$Tsubs)
  bind_rows(start, rest) 
}

# run subsidy simulation
sims_subs_abrupt <- subs_sim(list(benefit = params$benefit, 
                                  cost = Csubs1, 
                                  sigma = params$sigma, 
                                  r = params$r, 
                                  discount = params$discount, 
                                  Tsubs = Tsubs1, Tmax = params$Tmax))
sims_subs_sust <- subs_sim(list(benefit = params$benefit, 
                                cost = Csubs2, 
                                sigma = params$sigma, 
                                r = params$r, 
                                discount = params$discount, 
                                Tsubs = Tsubs2, Tmax = params$Tmax))

```


```{r res_subsidy, fig.cap = "Starting from the same initial states as Fig \\ref{fig:res_bimodal}, ES state timeseries are shown for (A) a large, abrupt subsidy (100\\% of adoption expenses are covered for two years) vs. (B) a smaller, more sustained subsidy (adoption cost is 80\\% of baseline for 10 years).  Ignoring discounting, subsidies have the same total cost to the funder (the equivalent of 2 years' worth of full adoption cost offsets).  After the subsidy period, agents adjust their decision rules to that of the base case (i.e. no subsidy) until $t = 20$.  (C) Shows that the sustained subsidy ultimately drove more DP adoption.", fig.width = 6, fig.asp = .5}
...
ggarrange(
  ...
  ggarrange(
    plt_sim_b_sweep(myopic_d_sims, title = ggtitle("B. Myopic decision-making"), 
                    tpos = "panel", col = pal[7], lmarmod = -3.5, rmarmod = -3.5, dnmarmod = -2.5) + 
      theme(legend.position="none"),
  ...
  ggarrange(
    plt_sim_b_sweep(chans_sims, title = ggtitle("C. Human/enviro. interaction"), 
                    tpos = "panel", col = pal[1], lmarmod = -7, dnmarmod = -2.5) + 
      theme(legend.position="none"),
