...
all_data <- read.csv("../external-data/ADAPT_CA-OR-WA_only_sw.csv")
...
dps_only <- all_data[,dp_varnames]
dps_only <- Hmisc::upData(dps_only, labels = dp_labs)
```
...
sim_plot <- function(sims, panelA_title = ggtitle(NULL), panelB_title = ggtitle(NULL), lab_lo_peak = FALSE, lab_hi_peak = FALSE, dnmarmod = 0, upmarmod = 0, lmarmod = 0, rmarmod = 0, cmarmod = 0){
   ...
   panelA <- df %>% 
     ...
     geom_path(alpha = 0.1, show.legend = FALSE) + 
      ...
      theme(axis.text.x=element_text(size=10),
            ...
            plot.margin=grid::unit(c(5+upmarmod,2+.5*cmarmod,5+dnmarmod,5+lmarmod), "mm"))
   ...
   panelB <- df %>% filter(time %in% c(1, Tmax))  %>%
     ...
     scale_y_continuous(breaks = scales::pretty_breaks(n = 2)) +
      theme(axis.text.x=element_text(size=10),
            ...
            plot.title = element_text(size = 10, face = "bold"),
            panel.grid.minor = element_blank(),
            plot.margin=grid::unit(c(5+upmarmod,5+rmarmod,5+dnmarmod,2+.5*cmarmod), "mm"))
...
```{r ca_wa_or_hist, message=FALSE, echo=FALSE}
# histogram of num dps used by a given farm
dp_counts <- rowSums(dps_only, na.rm = T)

dp_counts_df <- as.data.frame(dp_counts, row.names = NULL, optional = FALSE)
plot_data <- as.data.frame(table(dp_counts))

hst <- ggplot(plot_data, aes(x=dp_counts)) +
  geom_bar(aes(y=Freq), color=NA, fill="black", alpha = .7, stat = "identity") +
  ...
  theme(axis.text.x=element_text(size=10),
        ...
        plot.margin=grid::unit(c(9,5,5,5), "mm"))
...
```{r res_bimodal, fig.cap = "(A) Simulations of 500 model agents over ten farming seasons.  Initial ES states follow a truncated normal distribution (mean = 0.5; S.D. = 0.2; truncated at [0,1]).  Agents utilize decision policy $\\pi$ as shown in Fig \\ref{fig:res_policy} for nine timesteps.  Right plot shows initial distribution (dark blue) and bimodal density distribution at $t = 10$ (light blue), with peaks annotated. (B) Observed bimodal DP adoption distribution in our empirical data.", fig.width = 6, fig.asp = .5}

ggarrange(
     sim_plot(sims, lab_lo_peak = TRUE, lab_hi_peak = TRUE, upmarmod = 4, rmarmod = -5, cmarmod = -5), hst, labels = c("A. Model results", "B. Empirical distrib."), font.label = list(size = 10, face = "bold", family = "Roboto Condensed"), hjust = -.25, vjust = 2, widths = c(3, 1.1), nrow = 1, ncol = 2
   )
...
```{r ca_wa_or_tenure_plot, message=FALSE, echo=FALSE}
ten_plot <- function(plot_data, ylabel) {
  ggplot(plot_data, aes(x = cats, y = vals)) +
    geom_bar(width = .75, position=position_dodge(width=.84), stat="identity", color=NA, fill="black", alpha = .7,) +
    ...
    theme(axis.text.x=element_text(size=10, color = "black", family = "Roboto Condensed"),
          ...
          panel.grid.minor.x = element_blank(),
          plot.margin=grid::unit(c(9,5,2,5), "mm"))
}

# differences in n dps based on owned/leased land tenure
prop_owned <- all_data[,"FC1_Owned.ac"] / all_data[,"FC1_Total.ac"]
ownrshp <- data.frame(prop_owned)
ownrshp <- ownrshp %>%
    mutate(mo_ml = case_when(prop_owned >= .5 ~ 'Mostly Owned',
                                  TRUE ~ 'Mostly Leased'))
mo_ml <- ownrshp$mo_ml
moml_n_dps <- data.frame(mo_ml, dp_counts)
plot_data <- group_by(moml_n_dps, mo_ml) %>%
  summarize_all(mean, na.rm = TRUE)
names(plot_data)[1] <- "cats"
names(plot_data)[2] <- "vals"
p1 <- ten_plot(plot_data, "Mean Num. DPs")

# differences in prob. of using 1 or more dps based on owned/leased land tenure
dp_count_cum_probs <- data.frame(dp_counts) %>%
  mutate(one = case_when(dp_counts >= 1 ~ 1, TRUE ~ 0))
p_one_plus_dps <- dp_count_cum_probs$one
moml_p_one_plus_dps <- data.frame(mo_ml, p_one_plus_dps)
plot_data <- group_by(moml_p_one_plus_dps, mo_ml) %>%
  summarize_all(mean, na.rm = TRUE)
names(plot_data)[1] <- "cats"
names(plot_data)[2] <- "vals"
p2 <- ten_plot(plot_data, expression("Prob.">="1 DP"))
...
```{r res_tenure, fig.cap = "(A) Result of short land tenure on DEP adoption.  The simulation is identical to that in Fig \\ref{fig:res_bimodal}, but the MDP is solved under a finite, two-year time horizon. (B) Impact of land tenure on the probability of a farmer in our survey dataset adopting at least one DP.", fig.width = 6, fig.asp = .5}
# sim_plot(sims_short_tenure, lab_lo_peak = TRUE) 
ggarrange(
     sim_plot(sims_short_tenure, lab_lo_peak = TRUE, upmarmod = 4, rmarmod = -5, cmarmod = -5), p2, labels = c("A. Short-tenure model results", "B. Empirical data"), font.label = list(size = 10, face = "bold", family = "Roboto Condensed"), hjust = -.25, vjust = 2, widths = c(3, 1.1), nrow = 1, ncol = 2
   )
