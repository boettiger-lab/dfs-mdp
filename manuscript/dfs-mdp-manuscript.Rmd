...
```{r load_ca_wa_or_data, message=FALSE, echo=FALSE}

caorwa_data <- read.csv("../external-data/ADAPT_CA-OR-WA_only_sw.csv")
...
```{r res_bimodal, fig.cap = "(A) Simulations of 500 model agents over ten farming seasons.  Initial ES states follow a truncated normal distribution (mean = 0.5; S.D. = 0.2; truncated at [0,1]).  Agents utilize decision policy $\\pi$ as shown in Fig \\ref{fig:res_policy} for nine timesteps.  Center plot shows initial distribution (dark blue) and bimodal density distribution at $t = 10$ (light blue), with peaks annotated. (B) Shows the observed bimodal DP adoption distribution in our survey data.", fig.width = 6, fig.asp = .5}

ggarrange(
     sim_plot_ts(sims, title = ggtitle("A. Model results"), rmarmod = -5), sim_plot_dens(sims, title = ggtitle(""), lab_lo_peak = TRUE, lab_hi_peak = TRUE, lmarmod = -5), hst, widths = c(3, 2, 1.75), nrow = 1, ncol = 3
   )
...
```{r model_tenure_barplot, results="hide"}
thresh <- .15
...
ggarrange(
     ...
     sim_plot_dens_comp(simA, simB, label1 = "Abrupt\nsubsidy\nfinal", label2 = "Sustained\nsubsidy\nfinal", title = ggtitle("C. Final state comparison"), lmarmod = -5, rmarmod = -5),
