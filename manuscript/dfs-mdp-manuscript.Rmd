...
Agents’ initial ecosystem states were distributed normally around a mean of $s =0.5$. We find that after following the optimal decision strategy (infinite horizon) for 20 decision cycles, agents have largely settled into two stable ecosystem states, with some farms transitioning to more simplified (lower levels of ecosystem services) farming systems, and others to more diversified (higher levels ecosystem services) systems (\@ref(fig:res_bimodal)B&C). Further, we find strong path dependency, with only 17\% of agents who started in a simplified ($s<0.5$) state concluding in a diversified ($s>0.5$) state, and only 7\% initially in the diversified state transitioning to a simplified state.
